...
{- So, you're mystified by JSON decoding in Elm? No worries, let's try and make
   ...
   from a raw JavaScript value to an Elm type. So, think of a `Decoder x` as a function `JavaScriptValue -> Result String x`.
...
{- Once you think you're done, run the tests for this exercise from the root of
   ...
   - If you have installed `elm-test` globally:
        `elm test tests/Exercise01/Main.elm`

   - If you have installed locally using `npm`:
        `npm run elm-test tests/Exercise01/Main.elm`

   - If you have installed locally using `yarn`:
        `yarn elm-test tests/Exercise01/Main.elm`
-}
